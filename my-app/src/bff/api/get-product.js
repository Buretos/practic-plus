...
// Сначала смотрим есть ли товар. В случае отстуствия товара по заданному адресу команда fetch выдаёт промис успешного запроса, который обрабатывается обычным .then. Т.е. ошибка 300-х и 400-х ответов это не ошибка запроса сервера (когда, например сервер не ответил), а ошибка контента данных на серевре. Сервер же успешно обработал запрос. Так ошибка 404 (товар не найден) обрабатывается через then, а не через catch.
